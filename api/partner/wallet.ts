...
export default async function handler(
  req: {
    ...
    query?: { limit?: string; offset?: string; startDate?: string; finishDate?: string; intent?: string };
    body?: {
      value?: number;
  ...
  if (req.method === 'GET' && req.query?.intent === 'subscription') {
    if (partner.mode !== 'shop') {
      return res.status(403).json({ success: false, error: 'Apenas dono da loja pode gerenciar assinatura.' });
    }
    try {
      ...
      if (!subscriptionId) {
        ...
        const patch: Record<string, unknown> = { asaas_platform_subscription_id: subscriptionId };
        ...
        await supabase.from('shops').update({ asaas_customer_id: customerId }).eq('id', shopId);
      }
      ...
      console.error('[api/partner/wallet][subscription]', e);
