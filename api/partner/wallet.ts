...
export default async function handler(
  ...
  if (req.method === 'GET' && (req.query?.intent === 'subscription' || req.query?.intent === 'invoices')) {
    return res.status(410).json({
      success: false,
      error:
        'Fluxo descontinuado. Configure VITE_PLATFORM_SUBSCRIPTION_CHECKOUT_URL (e opcionalmente VITE_PLATFORM_SUBSCRIPTION_MANAGE_URL) no front.',
    });
  }
