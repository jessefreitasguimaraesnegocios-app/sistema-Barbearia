...
const Layout: React.FC<LayoutProps> = ({
  ...
  const navInactive =
    'text-[var(--app-text-muted)] hover:bg-[var(--app-nav-hover)] transition-all duration-200 ease-out';
  ...
  return (
    <div
      data-app-shell
      className="flex min-h-screen flex-col bg-transparent text-[var(--app-text)] transition-[color] duration-300 ease-out md:flex-row"
      style={
    ...
    >
      ...
      {user && user.role !== 'PENDING' && (
        <aside className="sticky top-0 hidden h-screen w-64 flex-col border-r border-[var(--app-border)] bg-[var(--app-sidebar-solid)] p-6 shadow-[var(--app-shadow-sm)] backdrop-blur-xl dark:border-[var(--app-border)] dark:bg-[color-mix(in_oklab,var(--app-sidebar-solid)_88%,transparent)] md:flex">
      ...
      {/* Mobile Top Header */}
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-[var(--app-border)] bg-[var(--app-sidebar-solid)]/95 p-4 shadow-sm backdrop-blur-xl dark:bg-[color-mix(in_oklab,var(--app-sidebar-solid)_92%,transparent)] md:hidden">
        ...
        {user && (
          <div className="flex items-center gap-2">
            <button
              ...
              className="relative rounded-xl p-2 text-[var(--app-text-muted)] transition-colors duration-200 hover:bg-[var(--app-nav-hover)] hover:text-[var(--app-text)]"
            >
              <i className="fas fa-bell text-xl"></i>
              {unreadCount > 0 && (
                <span className="absolute right-1.5 top-1.5 h-2 w-2 rounded-full border border-[var(--app-sidebar-solid)] bg-red-500 dark:border-zinc-900" />
              )}
            </button>
            <button
              onClick={onLogout}
              className="rounded-xl p-2 text-[var(--app-text-muted)] transition-colors duration-200 hover:bg-[var(--app-nav-hover)] hover:text-[var(--app-text)]"
            >
      ...
      {showNotifCenter && (
        <div className="animate-fade-in fixed inset-0 z-[1000] flex justify-end bg-black/40 backdrop-blur-sm dark:bg-black/60">
          <div className="glass-panel flex h-full w-full max-w-sm animate-slide-left flex-col border-l border-[var(--app-border)] shadow-2xl dark:border-[var(--app-border)] dark:bg-[color-mix(in_oklab,var(--app-elevated)_96%,transparent)]">
                ...
                notifications.map((n) => (
                  <div
                    ...
                    className={`rounded-2xl border p-4 transition-all duration-200 ${
                      n.read
                        ? 'border-[var(--app-border)] bg-[var(--app-elevated)] opacity-75 dark:bg-zinc-900/50'
      ...
      {user && user.role !== 'PENDING' && (
        <nav className="fixed bottom-0 left-0 right-0 z-50 flex justify-around border-t border-[var(--app-border)] bg-[var(--app-sidebar-solid)]/95 p-2 shadow-[var(--app-shadow-md)] backdrop-blur-xl dark:bg-[color-mix(in_oklab,var(--app-sidebar-solid)_94%,transparent)] md:hidden">
          {user.role === 'CLIENT' ? (
            <>
              <button
                onClick={() => onNavigate('client-home')}
            ...
            <>
              <button
                onClick={() => onNavigate('shop-dashboard')}
                className={`flex flex-col items-center rounded-xl p-2 transition-colors duration-200 ${
                  currentView === 'shop-dashboard' ? shopMobileActive : 'text-[var(--app-text-muted)]'
                }`}
              >
              ...
              <button
                onClick={() => onNavigate('shop-agenda')}
                className={`flex flex-col items-center rounded-xl p-2 transition-colors duration-200 ${
                  currentView === 'shop-agenda' ? shopMobileActive : 'text-[var(--app-text-muted)]'
                }`}
              >
                <i className="fas fa-calendar-alt text-xl"></i>
                <span className="mt-1 text-[10px]">Agenda</span>
              ...
              <button
                onClick={() => onNavigate('shop-wallet')}
                className={`flex flex-col items-center rounded-xl p-2 transition-colors duration-200 ${
                  currentView === 'shop-wallet' ? shopMobileActive : 'text-[var(--app-text-muted)]'
                }`}
              >
              ...
              <button
                onClick={() => onNavigate('shop-orders')}
                className={`flex flex-col items-center rounded-xl p-2 transition-colors duration-200 ${
                  currentView === 'shop-orders' ? shopMobileActive : 'text-[var(--app-text-muted)]'
                }`}
              >
                <i className="fas fa-shopping-bag text-xl"></i>
                <span className="mt-1 text-[10px]">Pedidos</span>
              ...
              {user.role === 'SHOP' && (
                <button
                  onClick={() => onNavigate('shop-customization')}
                  className={`flex flex-col items-center rounded-xl p-2 transition-colors duration-200 ${
                    currentView === 'shop-customization' ? shopMobileActive : 'text-[var(--app-text-muted)]'
                  }`}
                >
