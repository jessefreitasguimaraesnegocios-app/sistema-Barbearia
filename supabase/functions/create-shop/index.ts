...
const ALLOWED_MP_PREAPPROVAL_PLAN_IDS = new Set([
  "6711928bd21d4f3fb587c38925eef5ab",
  "4e35683f6b69425c93785bf3db667517",
]);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const {
      ...
      mercadoPagoPreapprovalPlanId: bodyMpPlanId,
    } = bodyJson as Record<string, unknown>;

    const mpPlanTrim =
      bodyMpPlanId != null && typeof bodyMpPlanId === "string" ? bodyMpPlanId.trim() : "";
    if (!mpPlanTrim || !ALLOWED_MP_PREAPPROVAL_PLAN_IDS.has(mpPlanTrim)) {
      return new Response(
        JSON.stringify({
          success: false,
          error: "Selecione o plano Mercado Pago da mensalidade (R$ 299,99 ou R$ 159,99).",
        }),
        {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const { data: shop, error: insertError } = await supabase
      ...
      .insert({
        ...
        mp_preapproval_plan_id: mpPlanTrim,
        pix_key: pixKeyTrim,
        rating: 5.0,
        profile_image,
        banner_image,
    ...
    return new Response(
      JSON.stringify({
        ...
        financePending: true,
        mercadoPagoPreapprovalPlanId: mpPlanTrim,
        message:
