-- Cliente autenticado pode consultar ocupação (horário + duração + profissional) para agendar, sem expor outros dados do agendamento.
...
REVOKE ALL ON FUNCTION public.get_shop_booking_blocks(uuid, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_shop_booking_blocks(uuid, date) TO authenticated;
